 ...
 **/
function AmbientLightSensor(options) {
  this.state = 'idle';
  this.type = 'ambientlight';
  this.properties = ['illuminance'];
  this.illuminance = 0;
...
AmbientLightSensor.prototype.update = function update() {
  var self = this;
  try {
...
AmbientLightSensor.prototype.stop = function stop() {
  if ( this.state === 'idle' ) return;
  this.interval = clearInterval(this.interval);
  this.state = 'idle';
}

AmbientLightSensor.prototype.start = function start() {
  var self = this;
...
if (module.parent === null) {
  var config = process.argv[2]
      ...
      : null;
  var sensor = new AmbientLightSensor(config);
